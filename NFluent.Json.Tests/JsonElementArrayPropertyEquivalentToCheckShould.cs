...
public class JsonElementArrayPropertyEquivalentToCheckShould
{
    ...
    public async Task HasArrayPropertyEquivalentToWorksOnHavingPropertyWithExpectedNumbers(int[] value, IEnumerable<int> equivalentValue)
    {
    ...
    [InlineData(new[] { "1", "2" }, new[] { "1", "2" })]
    [InlineData(new[] { "1", "2" }, new[] { "2", "1" })]
    public async Task HasArrayPropertyEquivalentToWorksOnHavingPropertyWithExpectedStrings(string[] value, IEnumerable<string> equivalentValue)
    {
        var json = await TestJson.Element(new { propA = value });
    ...
    public async Task HasArrayPropertyEquivalentToWorksOnHavingPropertyWithExpectedBooleans(bool[] value, IEnumerable<bool> equivalentValue)
    {
        var json = await TestJson.Element(new { propA = value });
    ...
    public async Task HasArrayPropertyEquivalentToWorksOnHavingPropertyWithExpectedNullValues(string?[] value, IEnumerable<string?> equivalentValue)
    {
        var json = await TestJson.Element(new { propA = value });
    ...
    [InlineData(new[] { 1, 2 }, new[] { 1, 2 })]
    [InlineData(new[] { 1, 2 }, new[] { 2, 1 })]
    public async Task HasArrayPropertyEquivalentToWorksOnHavingPropertyWithObjects(int[] ids, IEnumerable<int> equivalentIds)
    {
